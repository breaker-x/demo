...
public class PageInterceptor implements Interceptor {
		...
		if (ivk.getTarget() instanceof RoutingStatementHandler) {
			...
			if (param instanceof HashMap) {
				...
				if (map.containsKey(PAGE_KEY)) {
					...
					p.setTotalRecords(queryTotal(ivk, ms, bs, param, sql));
					set(p);
	...
	private String pageSql(String sql, Page page) {
		...
		if(page.isNoPaging()){
			...
			sb.append(sql);
			sb.append(") tmp_tb order by "+page.getSort()+" "+page.getDir());
			return sb.toString();
		}
		if ("hsqldb".equals(dialect)) {
			...
			sb.append("select * from(");
			sb.append(sql);
			sb.append(") tmp_tb order by "+page.getSort()+" "+page.getDir()+") tmp_tb");
			sb.append(" limit " + page.getStartIndex() + ","
					+ page.getPageSize());
		} else if ("oracle".equals(dialect)) {
			sb.append("select * from (select tmp_tb.*,ROWNUM row_id from (");
			sb.append("select * from (");
			sb.append(sql);
			sb.append(")order by "+page.getSort()+" "+page.getDir());
